...
I used the provided dataset provided by the Black Forest Mountain Rescue Team. The dataset consists of
- 166 training images: all are normal (without a missing person)
- 60 test images: 30 are normal (without a missing person) and 30 are abnormal (with a missing person)

The images had no GPS location in their metadata. As the location is fundamental information, I generated random GPS coordinates within the Black Forest in Germany for each image, and attached these as metadata to the images.

## 2.2 Modeling Approach

I tackled this challenge using a "predict-then-optimize" approach:

1. **Feature Extraction**: I used a pre-trained convolutional neural network (CNN) model, specifically ResNet18, to extract image embeddings from the drone footage. In particular, I used the penultimate layer of the ResNet18 model as the feature extractor. This layer outputs a 512-dimensional vector for each image, which serves as a compact representation of the image's content.
